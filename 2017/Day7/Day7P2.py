...
class Node():
    nodes = {}
    def __init__(self,name,weight,childrens):
        self.name = name
        self.weight = weight
        self.parent = None
        if childrens:
            self.childrens = childrens.split(', ')
        else:
            self.childrens = None
        self.total_weight = weight
        self.childrens_weight = []
        Node.nodes[self.name] = self
      
    def find_childrens_total_weight(self):
        if self.childrens:
            for each in self.childrens:
                self.total_weight = self.total_weight + (Node.nodes[each].find_childrens_total_weight() or 0 )
                self.childrens_weight.append(Node.nodes[each].total_weight)
        return self.total_weight
    
    def find_unbalanced_child(self):
        if self.childrens:
            diff_weights = [True if self.childrens_weight.count(x) == 1 else False for x in self.childrens_weight]
            if any(diff_weights):
                return Node.nodes[self.childrens[diff_weights.index(True)]].find_unbalanced_child()
        return(self.name, self.weight, self.total_weight, self.parent.childrens_weight)
...
def solve(x):
    ...
    base_node = None
    tail = None 
    for each_line in x.split('\n'):
        try:
            front, tail = each_line.split(' -> ')
        except ValueError:
            front = each_line.split(' -> ')[0]
            tail = None 
        
        name, weight = front.split(' (')
        
        Node(name, int(weight.strip(')')), tail)
        
        if tail:
            for each in tail.split(', '):
                if each in Node.nodes.keys():
                    Node.nodes[each].parent = Node.nodes[name]
                else:
                    to_be_processed.append((name,each))
      
    for parent,child in to_be_processed:
        Node.nodes[child].parent = Node.nodes[parent]
    
    for each in Node.nodes.values():
        if not each.parent:
            base_node = each
    base_node.find_childrens_total_weight()
    issue_node_name, amount, total_amount, list_of_weights = base_node.find_unbalanced_child()
    
    for each in list_of_weights:
        if each != total_amount:
            return amount - (total_amount - each)
    return None
