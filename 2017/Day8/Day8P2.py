...
def solve(x):
    ...
    for each_line in x.split('\n'):
        reg1, command, value1, _, reg2, cond, value2 = each_line.split(' ')
        if eval(str(register[reg2])+cond+value2):
            if command == 'inc':
                register[reg1] += int(value1)
            else:
                register[reg1] -= int(value1)
            if register[reg1] > max_value:
                max_value = register[reg1]
